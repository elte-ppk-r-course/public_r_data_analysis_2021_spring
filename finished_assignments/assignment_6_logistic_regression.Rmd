...
clean_titanic_dataset <- titanic_dataset %>%
  select(Survived, Age, Sex, Pclass, SibSp, Parch, Family) %>%
...
```{r}
model1 <- glm(Survived ~ Age + Sex + Pclass + SibSp, data = clean_titanic_dataset, family = binomial)
model2 <- glm(Survived ~ Age +Sex + Pclass + Family, data = clean_titanic_dataset, family = binomial)
