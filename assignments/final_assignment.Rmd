...
rise_and_fall <- emperors_new %>%
  count(rise, cause)

rise_and_fall2 <- emperors_new %>%
...
``` {r}
age_and_long <- emperors_new %>%
  ggplot(aes(age_reign_start, reign_long_y, color = rise)) +
  geom_point() +
  geom_smooth(
     method = "lm", 
     color = "blue",
...
death_and_long <- emperors_new %>%
  ggplot(aes(age_reign_end, reign_long_y, color = cause)) +
  geom_point() +
  geom_smooth(
     method = "lm", 
     color = "blue",
...
died_by2 <- emperors_recoded %>%
  filter(cause != "NA") %>%
  ggplot(aes(cause_recoded, fill = killer_recoded)) +
  geom_bar(width = 0.3) +
  coord_flip() +
  theme_light() +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  xlab(NULL) +
  ylab(NULL) +
  labs(title = "The Cause of Death Among the Roman Emperors in the Principate Era") +
  scale_fill_viridis(discrete = TRUE, na.value = "grey")

died_by2

killer_and_rise1 <- emperors_recoded %>%
  ggplot(aes(killer_recoded, fill = rise)) +
  geom_bar(width = 0.3) +
  coord_flip() +
  theme_light() +
  theme(legend.position = "bottom") +
  xlab("The killer of the emperors") +
  ylab(NULL) +
  labs(title = "The Rise and Killing of the Roman Emperors in the Principate Era #1") +
  scale_fill_viridis(discrete = TRUE, na.value = "grey", name = "The rise of the emperors")

killer_and_rise1

killer_and_rise2 <- emperors_recoded %>%
  ...
  ylab(NULL) +
  labs(title = "The Rise and Killing of the Roman Emperors in the Principate Era #1") +
  scale_fill_viridis(discrete = TRUE, na.value = "grey", name = "The killer of the emperors")
...
```{r}
emperors_rise <- emperors_recoded %>%
  filter(rise == "Purchase") %>%
  select(index, name)

emperors_rise

emperors_recoded_new <- emperors_recoded %>%
  slice(-c(20)) %>%
  select(-cause, -killer, - birth_y, -death_y) %>%
  select(index, name, rise, cause_recoded, killer_recoded, everything())
