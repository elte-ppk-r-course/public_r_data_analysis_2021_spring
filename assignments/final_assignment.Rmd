...
#Making dummy variables from the cause_recoded for the models
  ...
  dplyr:: select(index, name, rise_recoded, cause_recoded, killer_recoded, everything(), -cause, -killer, - birth_year, -death_year, -rise) %>%
...
``` {r}
sjPlot:: tab_model(model_natural_null, model_natural, show.ci = FALSE, show.est = TRUE, show.fstat = TRUE, show.aic = TRUE, show.loglik = TRUE)
...
#plotting the model
model_natural_plot <- emperors_principate %>%
  ggplot(aes(age_reign_start, died_naturally, color = rise_recoded)) +
  geom_point(alpha = 0.2, color = "red") +
  geom_smooth(
     method = "glm", 
     color = "blue",
  ...
  facet_wrap(vars(rise_recoded)) +
  theme_light() +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  ylab("Died by a natural death") +
  xlab("The age of emperors when their reign started")
