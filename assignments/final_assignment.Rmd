...
rise_emperors <- emperors %>%
  ggplot(aes(reign_start_year, rise, color = dynasty, shape = era)) +
...
``` {r}
emperors_principate %>%
  count(reign_long_year, died_by_force, died_by_battle, died_naturally) %>%
  view()

model_forced <- glm(died_by_force ~ rise + reign_long_year + age_reign_start, data = emperors_principate, family = "binomial")

summary(model_forced)

model_natural <- glm(died_naturally ~ rise + age_reign_start + reign_long_year, data = emperors_principate, family = "binomial")

summary(model_natural)

model_battle <- glm(died_by_battle ~ rise + reign_long_year + age_reign_start, data = emperors_principate, family = "binomial")
...
``` {r}
emperors_principate_forced <- emperors_principate %>%
  filter(cause_recoded == "Forced")

model_killer <- glm(died_by_force ~ killer_recoded + reign_long_year + age_reign_start, data = emperors_principate, family = "binomial")

summary(model_killer)
