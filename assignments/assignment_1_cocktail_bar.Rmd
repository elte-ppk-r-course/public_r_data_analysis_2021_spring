...
```{r}
cocktail_data <- readr:: read_tsv(url("https://raw.githubusercontent.com/nthun/cocktail-balance/master/cocktail_data.tsv"), na = c(" ", "NA", "not yet"))
```
...
cocktail_data_clean <- cocktail_data %>%
  separate_rows(ingredients, sep = "<br/>|<br>|<b4>|<b>|\\sor\\s") %>%
  mutate(ingredients,
...
Kiszedtem változókká az ingredients-et és a name-t.

```{r}
names_cocktail <- cocktail_data_clean %>%
  count(name) %>%
  select(name) %>%
  pull(name)

names_ingredients <- cocktail_data_clean %>%
  count(ingredients) %>%
  select(ingredients) %>%
  mutate(ingredients, ingredients = as.character(ingredients)) %>%
  pull(ingredients)
```
