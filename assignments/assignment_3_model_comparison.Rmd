...
# Convert the dataset in data frame for further anaylsis
...
# Tolerane and Variance Inflation Factor
ols_vif_tol(c_model_no)

# Cortisol serum (6.96) and cortisol saliva (7.53) have high indexes of multicollarity, this means that they are highly correlated predictors, its necessary to remove one for a better model fit. 
...
c_f_sm <- coef(summary(s_model2))
print(c_f_sm)

c_i_sm <- confint(s_model2, level = 0.95)
print(c_i_sm)

b_c_sm <- standardCoefs(s_model2)
print(b_c_sm)

sm_report_t <- cbind(c_f_sm, c_i_sm, b_c_sm)
...
Interpreation: Based on the coefficients, we see that age and pain are negative correlated (b = -0,076; p = 0.0007), as Age increases, Pain decreases.
...
The complex model explained 42 % of variance. (F 6, 142 = 18.52 p.< 7.86)
